...
def main(unparsed):
  ...
  if FLAGS.mode == 'train':
    ...
    estimator = tf.estimator.Estimator(
      ...
      params=params)
    for _ in range(0, params['train_epochs'] // params['eval_frequency']):
      tensors_to_log = {
      ...
      result_iter = estimator.predict(lambda: input_fn(params, 'train', params['data_dir'], _NUM_SAMPLES['train']))
      ...
      tf.logging.info('train pairwise accuracy = {0}'.format(pairwise_acc))
      tf.logging.info('train mean squared error = {0}'.format(mse))
